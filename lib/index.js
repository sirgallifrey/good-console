...
internals.utility = {
    ...
    formatResponse(event, tags, settings) {
        ...
        const headers = event.headers ? ` headers: ${SafeStringify(event.headers)}` : '';
        const requestPayload = event.requestPayload ? ` requestPayload: ${SafeStringify(event.requestPayload)}` : '';
        const responsePayload = event.responsePayload ? ` responsePayload: ${SafeStringify(event.responsePayload)}` : '';
        ...
        // method, pid, error
        const output = `${event.instance}: ${method} ${event.path} ${query} ${statusCode} (${event.responseTime}ms)${headers}${requestPayload}${responsePayload}`;
